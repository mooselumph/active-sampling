...
class FlowStep(nn.Module):
    ...
    def __call__(self, x, logdet=0, reverse=False):
        ...
        if not reverse:
            ...
            x, logdet = self.coupling(out_dims, self.subnet)(x, logdet=logdet, reverse=False)
        else:
            x, logdet = self.coupling(out_dims, self.subnet)(x, logdet=logdet, reverse=True)
