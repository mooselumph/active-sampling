...
def train_step(model, unflattener, state, xb, yb, alpha):
    ...
    def loss_fn(params):
        base_param_pred = model.apply(params, xb).flatten()
...
def train_and_evaluate(key, config, model, trainloader, testloader, MLP_unflattener):
    ...
    for epoch in range(config.train.epochs):
        ...
        for xb, yb in trainloader:
            state, batch_loss = train_step(model, MLP_unflattener, state, xb.numpy(), yb.numpy(), config.train.alpha)
